	...
	<div class="wrap-cart d-flex align-items-stretch justify-content-between">
			...
			<div class="top-cart">
				...
				<div class="list-procart">
					...
					<?php $max = count($_SESSION['cart']); for($i=0;$i<$max;$i++) {
						...
						$mau = (!empty($_SESSION['cart'][$i]['mau']))?$_SESSION['cart'][$i]['mau']:0;
						$size = (!empty($_SESSION['cart'][$i]['size']))?$_SESSION['cart'][$i]['size']:0;
						$code = (!empty($_SESSION['cart'][$i]['code']))?$_SESSION['cart'][$i]['code']:'';
		...
		<?php } else { ?>
			<a href="" class="empty-cart text-decoration-none">
				...
				<div class="btn btn-warning"><span><?=vetrangchu?></span></div>
			</a>
		<?php } ?>
	</div>
</form>