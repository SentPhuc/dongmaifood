...
class PDODb
{
        ...
        public function rawQuery($query, $params = null)
        {
            ...
            if($this->lastErrorCode != ""){
               $arr_error = $this->lastError;
               if($arr_error[2] != NULL)
               {
                   $arr_query = json_encode($this->lastQuery);
                   $resultError = '<script type="text/javascript">';
                   $resultError .= 'var Error = {Error : '.json_encode($arr_error[2]).', SQL : '.json_encode($arr_query).'};';
                   $resultError .= 'console.error(Error);';
                   $resultError .= '</script>';
                   echo $resultError;
               }
            }
        ...
        public function createlogs($user='Unnamed',$act,$com,$id)
        {
            ...
            fclose($file);
            // Kiá»ƒm tra vĂ  xĂ³a file log cÅ© quĂ¡ 90 ngĂ y
