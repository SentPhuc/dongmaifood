		...
		<div class="login-box">
			<div class="card">
				<div class="card-body login-card-body">
					...
					<!-- Main Footer -->
					<div class="info-login text-md mt-3 text-center text-sm">
						<?php if (!empty($setting['tenvi'])) {?>
							<p class="mb-1"><?=$setting['tenvi']?></p>
						<?php } ?>
						<?php if (!empty($optsetting['diachi'])) {?>
							<p class="mb-1">Địa chỉ: <?=$optsetting['diachi']?></p>
						<?php } ?>
						<?php if (!empty($optsetting['dienthoai'])) {?>
							<p class="mb-1">Tel: <?=$optsetting['dienthoai']?></p>
						<?php } ?>
						<?php if (!empty($optsetting['email'])) {?>
							<p class="mb-0">Email: <?=$optsetting['email']?></p>
						<?php } ?>
					</div>
				</div>
			</div>
		</div>
		<div class="login100-more" style="background-image: url('assets/images/xbg.jpg.pagespeed.ic.tiVxeakBSd.webp');"></div>
	</div>
</div>
<!-- Login js -->
